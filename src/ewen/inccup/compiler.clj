...
(defmethod compile-element ::literal-tag-and-attributes
  ...
  (let [[tag attrs _] (normalize-element [tag attrs])]
    `[~tag ~(compile-attr-map attrs) ~@(compile-seq content)]))
...
(defmethod compile-element ::literal-tag
  ...
  (let [[tag tag-attrs _] (normalize-element [tag])
    ...
    `(let [~attrs-sym ~attrs]
       (if (map? ~attrs-sym)
         [~tag (#'render-attr-map (merge ~tag-attrs ~attrs-sym))
          ~@(compile-seq content)]
...
(defn- compile-seq
  ...
  [content]
  (doall `((ewen.inccup.core/down-node!)
           ~@(->> (for [expr content]
                    (cond
...
(defmacro html
  "Render Clojure data structures to an internal representation
  to be processed by some incremental dom code."
  [& content]
  `(~'specify! (fn [] ~@(compile-seq content)) ewen.inccup.core/Html))

(comment
  (macroexpand '(html [:div {:e "e"}]))
